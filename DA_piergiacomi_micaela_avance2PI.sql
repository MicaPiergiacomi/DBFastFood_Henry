/*AVANCE 2  - MICAELA PIERGIACOMI*/

-- Selecciono la base que voy a trabajar
...
SELECT * FROM Ordenes; -- Visualizo la tabla
...
SELECT * FROM Empleados; -- Visualizo la tabla

SELECT COUNT (EmpleadoID) AS EmpleadosDepartamento, Departamento
FROM Empleados
GROUP BY Departamento;

SELECT Departamento, COUNT(*) AS EmpleadosDepartamento
FROM Empleados
GROUP BY Departamento;
...
SELECT COUNT(ProductoID) AS CantidadProductos, CategoriaID
FROM Productos
...
SELECT COUNT (ClienteID) AS CantitdadClientes
FROM Clientes;

/*5	Análisis de desempeño de sucursales
Pregunta: ¿Cuáles son las sucursales con un promedio de ventas por orden superior a un valor específico, ordenadas por el promedio de kilómetros recorridos para las entregas de mayor a menor?*/

SELECT SucursalID,
	AVG (TotalCompra) AS PromedioTotalCompra,
	AVG (KilometrosRecorrer) AS PromedioKilimetrosRecorridos
FROM Ordenes
GROUP BY SucursalID
HAVING AVG(TotalCompra) > 50
ORDER BY PromedioKilimetrosRecorridos DESC;

-- MODIFICACION DE COLUMNA EsExterno DE TABLA Mensajeros

ALTER TABLE Mensajeros
ALTER COLUMN EsExterno BIT;